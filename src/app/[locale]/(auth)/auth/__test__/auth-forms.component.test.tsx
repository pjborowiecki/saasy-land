...
import type * as AuthClient from "~/src/modules/identity-access/infrastructure/auth/auth._client"
...
function setupSignOutButtonMocks(): void {
  ...
  signOutMock.mockImplementation(async (options) => {
    await invokeFetchOnSuccess(options?.fetchOptions)
  })
}
...
vi.mock(
  import("~/src/modules/identity-access/infrastructure/auth/auth._client"),
  async (importOriginal): Promise<Partial<typeof AuthClient>> => {
